...
MYSQL_USER = os.getenv('MYSQL_USER', 'viewkcs')
MYSQL_PASS = os.getenv('MYSQL_PASSWORD', 'viewkcs@2024')
MYSQL_DB = os.getenv('MYSQL_DB', 'bkmis_kcshpsdq')
MYSQL_TABLE = os.getenv('MYSQL_TABLE', 'view_dq1_lg_daura_lc4')
...
def get_last_timestamp():
    ...
    try:
        with open(STATE_FILE_PATH, 'r') as f:
            ...
            if not timestamp:
                # Nếu file trống, trả về mốc thời gian mặc định
...
def poll_database_and_publish():
    ...
    try:
        ...
        mysql_conn = mysql.connector.connect(
            host=MYSQL_HOST,
            port=3307, # chỉ sử dụng khi cần chỉ định port
            user=MYSQL_USER,
            password=MYSQL_PASS,
            database=MYSQL_DB
        )
        ...
        rabbitmq_conn = pika.BlockingConnection(pika.ConnectionParameters(host=RABBITMQ_HOST, credentials=credentials)) # chỗ này có thể thêm port = ... nếu port khác 5672
